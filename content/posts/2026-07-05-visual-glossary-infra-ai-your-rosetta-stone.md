...
Final post in the series. In the [previous one](/2026/07/01/ai-adoption-framework-from-enthusiasm-to-governance/), we built the 6-phase adoption framework. Now: your permanent quick-reference card.

You already speak infrastructure fluently. AI isn't a foreign language; it's a dialect. This glossary maps each AI term to something you already understand.

## How to use this

Every entry has: the **AI term**, the **infra analogy** in parentheses, a **concise definition**, and **when you'll encounter it** in your work. Organized into 6 categories. Pin this page.
...
| **Parameters / Weights** | Configuration values | The internal numerical values that define how the model processes input and generates output. GPT-4 has trillions. | Sizing infra: more parameters = more memory, more compute, more storage |
| **Epoch** | Full backup cycle | One complete pass through the entire training dataset. Models typically train for tens to hundreds of epochs. | Estimating duration and cost of training jobs |
...
| **NVLink** | GPU-to-GPU interconnect | High-speed link connecting GPUs within a single node. ~10x the bandwidth of PCIe. | Sizing multi-GPU VMs: GPUs with NVLink share data fast enough to act as unified memory |
...
| **PTU** | Reserved capacity (like reserved instances) | Pre-allocated, guaranteed compute capacity for Azure OpenAI models. Consistent latency and throughput. | When workloads need predictable performance: PTU eliminates throttling at a fixed cost |
...
| **LoRA** | Lightweight fine-tuning | Technique that trains a small adapter layer (~1-2% of parameters) instead of the entire model. | When teams want to customize a foundation model without the cost of full fine-tuning |
...
| **Prompt Injection** | SQL injection for AI | Attack where untrusted input is crafted to override model instructions, causing unintended behavior. | Securing AI endpoints exposed to user input. The #1 security concern for LLM applications |
| **ZeRO** | Memory optimization for distributed training | Family of techniques that partitions optimizer states, gradients, and parameters across GPUs to eliminate redundancy. | Training large models that don't fit in GPU memory even with data parallelism. Standard solution in DeepSpeed |

## Quick reference: top 20 terms

Pin this card. It's your Rosetta Stone.
...
| 15 | **PTU** | Reserved capacity (like reserved instances) |
| 16 | **CUDA** | NVIDIA's GPU SDK/instruction set |
| 17 | **LoRA** | Lightweight fine-tuning (<1% of parameters) |
...
15 posts. From the first concept (why infrastructure engineers matter for AI) to this complete glossary. If you followed along, you now have a solid foundation for operating AI workloads in production with security, cost efficiency, and governance.

The full book, in English, is available for free at [ai4infra.com](https://www.ai4infra.com). If you found this series useful, share it with other infra professionals who are making the transition to AI.

AI isn't a revolution that replaces what you know. It's an extension. Your skills in networking, storage, compute, security, and automation are exactly what AI projects need to work in production. The difference is that now you have the vocabulary and the patterns to connect both worlds.

---

*This is post 15 of the series "AI for Infrastructure Engineers", based on the book [AI for Infrastructure Professionals](https://ai4infra.com):*
...
15. **Visual Glossary: Your Rosetta Stone**
